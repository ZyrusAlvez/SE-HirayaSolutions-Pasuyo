...
import { View, Text, FlatList, TextInput, TouchableOpacity, RefreshControl, Platform, ActivityIndicator } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { useRouter } from 'expo-router';
import { getReports, deleteErrandAdmin, updateUserActiveStatus } from '@/controllers/adminController';
import type { AdminReport } from '@/controllers/adminController';
import { ERRAND_REASONS, USER_REASONS } from '@/controllers/reportController';
import Dropdown from '@/view/components/Dropdown';
import TabToggle from '@/view/components/TabToggle';
import KebabMenu from '@/view/components/KebabMenu';
import ReasonModal from '@/view/components/ReasonModal';
...
export default function ReportsScreen() {
  ...
  const handleDelete = async (reason: string) => {
    if (!deleteTarget?.errand_id) return;
  ...
  const handleSuspend = async (reason: string) => {
    if (!suspendTarget?.reported_id) return;
    setSuspendVisible(false);
    setActionLoading(true);
    const result = await updateUserActiveStatus(suspendTarget.reported_id, true, reason);
    setActionLoading(false);
    if (!result.success) { toast({ title: result.error, preset: 'error' }); return; }
  ...
  return (
    <View style={[{ flex: 1, backgroundColor: '#F9FAFB' }, Platform.OS === 'web' && { maxWidth: 1200, width: '100%', alignSelf: 'center' as const }]}>
          ...
          <View style={{ backgroundColor: 'white', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: '#F3F4F6', flexDirection: 'row', alignItems: 'center' }}>
            ...
            {tab === 'user' && (
              <KebabMenu actions={[
                ...
                { label: 'Suspend', icon: 'ban-outline', onPress: () => { setSuspendTarget(item); setSuspendVisible(true); } },
              ]} />
            )}
          </View>
        )}
            ...
            <View style={{ gap: 8 }}>
              {[1, 2, 3, 4].map(i => (
                <View key={i} style={{ backgroundColor: 'white', borderRadius: 16, padding: 12, borderWidth: 1, borderColor: '#F3F4F6', gap: 6 }}>
                  ...
                  <View style={{ width: '30%', height: 12, borderRadius: 6, backgroundColor: '#E5E7EB' }} />
                </View>
              ))}
            </View>
          ) : (
      ...
      <ReasonModal
        ...
        description={`Select a reason for suspending ${suspendTarget?.reported_name}.`}
