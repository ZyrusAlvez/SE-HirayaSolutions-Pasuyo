...
export default function AccountsChartsPanel({ verifiedCount, unverifiedCount, totalCount, activities, chartWidth }: Props) {
  ...
  return (
    <View style={{ gap: 16 }}>
      {totalCount > 0 && (
        <View style={{ backgroundColor: 'white', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#F3F4F6', alignItems: 'center' }}>
          ...
          <Text style={{ fontSize: 12, color: '#6B7280', marginBottom: 8 }}>
            {verifiedCount} verified · {unverifiedCount} unverified · {totalCount} total
          </Text>
          <PieChart
            data={pieData}
            width={chartWidth}
      ...
      <View style={{ backgroundColor: 'white', borderRadius: 16, padding: 16, borderWidth: 1, borderColor: '#F3F4F6' }}>
        ...
        <SimpleBarChart data={lineData.data} labels={lineData.labels} height={140} />
      </View>
    </View>
  );
}
