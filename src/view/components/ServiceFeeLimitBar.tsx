...
export default function ServiceFeeLimitBar({ totalFees, isVerified, isAdmin }: Props) {
  ...
  const hasCredit = totalFees < 0;
  const displayFees = Math.max(totalFees, 0);
  const ratio = Math.min(displayFees / limit, 1);
  const percentage = Math.round(ratio * 100);
  const remaining = Math.max(limit - displayFees, 0);

  const barColor = hasCredit ? '#3B82F6' : ratio >= 1 ? '#EF4444' : ratio >= 0.75 ? '#F59E0B' : '#10B981';
  const bgTint = hasCredit ? '#EFF6FF' : ratio >= 1 ? '#FEF2F2' : ratio >= 0.75 ? '#FFFBEB' : '#F0FDF4';
  const textColor = hasCredit ? '#1E40AF' : ratio >= 1 ? '#991B1B' : ratio >= 0.75 ? '#92400E' : '#065F46';
  ...
  return (
    <View style={{ backgroundColor: 'white', borderRadius: 14, padding: 14, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 1 }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <Text style={{ fontSize: 11, color: '#6B7280' }}>{hasCredit ? `₱${Math.abs(totalFees).toLocaleString()} credit` : `₱${displayFees.toLocaleString()} used`}</Text>
      ...
      <View style={{ backgroundColor: bgTint, borderRadius: 8, padding: 10, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        ...
        <Text style={{ fontSize: 11, color: textColor, flex: 1, lineHeight: 16 }}>
          {hasCredit
            ? `${isAdmin ? 'User has' : 'You have'} ₱${Math.abs(totalFees).toLocaleString()} in credit from overpayment.`
            : ratio >= 1
