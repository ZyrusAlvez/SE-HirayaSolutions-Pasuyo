    ...
    <View style={{ flexDirection: 'row', marginHorizontal: 24, marginTop: 16, marginBottom: 8, backgroundColor: '#F3F4F6', borderRadius: 12, padding: 4 }}>
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <Ionicons name={tab.icon as any} size={isSmall ? 14 : 15} color={active ? '#111827' : '#9CA3AF'} />
            <Text numberOfLines={1} style={{ fontSize: isSmall ? 11 : 13, fontWeight: '600', color: active ? '#111827' : '#9CA3AF', flexShrink: 1 }}>{tab.label}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
